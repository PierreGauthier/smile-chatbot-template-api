...
class LangsmithSearchResponseLotOneFilterPromptProvider(RagMainPromptProvider):

    def __init__(self, settings: Annotated[Settings, Depends(get_settings)]):
        self.client = Client(api_key=settings.langchain_api_key)
        self.prompt_name = settings.elastic_suite_search_response_lot_one_filter
    ...
    def get_prompt(self, search_context:SearchContext) -> ChatPromptTemplate:
        ...
        rag_prompt = rag_prompt.partial(
            ...
            used_filter=search_context.search_used_filters[0].label,
