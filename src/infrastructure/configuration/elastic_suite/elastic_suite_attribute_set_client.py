...
class ElasticSuiteAttributeSetClient:
    def __init__(
            self, 
        ...
        self.base_client = BaseClient(settings.elastic_suite_api_base_url)
    ...
    def get_attribute_set(self, params: AttributeSetApiParam) -> AttributeSetApiResponse:
        url = f"{self.base_client.api_base_url}/{self.settings.elastic_suite_attribute_set_endpoint}?page={params.page}&pageSize={params.page_size}"
        query = f"GET /{self.settings.elastic_suite_attribute_set_endpoint}?page={params.page}&pageSize={params.page_size}"
        (x_correlation_id_key, x_correlation_id_value) = self.base_client.create_x_correlation_id()
        (content_type_key, content_type_value) = self.base_client.create_json_content_type()
        (basic_auth_key, basic_auth_value) = self.base_client.create_basic_auth(
        ...
        # print(url)
        response = self.base_client.get(url=url, headers=headers)
        params.query = query
